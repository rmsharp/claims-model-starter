...
class _FlakyClient:
    """Fake that always raises RepoClientError on create_project."""

    def create_project(
        self, *, namespace: str, name: str, visibility: str
    ) -> ProjectInfo:
        raise RepoClientError("simulated auth failure")
...
class _CommitFlakyClient:
    ...
    def commit_files(
        ...
        *,
        ...
        raise RepoClientError("simulated commit failure")


class TestCreateProject:
    def test_happy_path(
        self, fake_client: FakeRepoClient, intake_report: Any, data_report: Any, repo_target: Any
    ) -> None:
    ...
    def test_suffix_chain_all_taken_fails(
        ...
        client = FakeRepoClient(existing_names=existing)
        nodes = make_nodes(client)
        state = initial_state(
            intake_report=intake_report.model_dump(mode="json"),
            data_report=data_report.model_dump(mode="json"),
    ...
    def test_non_conflict_repo_error_halts(
        self,
        intake_report: Any,
        data_report: Any,
        repo_target: Any,
        ...
        delta = nodes["create_project"](state)
        assert delta["status"] == "FAILED"
        assert "repo_error" in (delta.get("failure_reason") or "")
...
class TestBuildRepoProjectResult:
    ...
    def test_complete_state_produces_valid_result(
        ...
        result = build_repo_project_result(state)  # type: ignore[arg-type]
