...
- [x] **cli.py GitLab default URL inconsistency** (Session 31 F5) — Session 32 bumped `GITLAB_DEFAULT_HOST_URL` at `cli.py:39` from `"https://gitlab.example.com"` to `"https://gitlab.com"` (matches `orchestrator/config.py:33` `DEFAULT_GITLAB_URL`). Restructured `test_cli_host_gitlab_with_token_invokes_python_gitlab_adapter` to mirror the GitHub sibling's default-path shape (omits `--host-url`, asserts `https://gitlab.com`) and added `test_cli_host_gitlab_explicit_host_url_override` to preserve the explicit-override coverage. 444 → 445 tests. See `CHANGELOG.md`.
