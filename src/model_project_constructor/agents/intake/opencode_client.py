"""OpenCode-CLI-backed :class:`IntakeLLMClient` for the Intake Agent.
...
deliberate. The base class is this package's **prompt-and-parse** layer that
happens to carry an Anthropic transport; the subclass replaces *only* the
transport. Every prompt, every JSON-shape instruction, the ``_build_draft`` /
``_build_governance`` constructors and the fenced-text ``_extract_json`` are
**inherited unchanged**, so prompt drift across providers is structurally
impossible — exactly the argument that justifies ``bedrock_client.py`` (spec D1).

**The system prompt is folded into the user message (spec D2).** ``opencode run``
has **no** ``--system`` flag, so each call sends ``system + "\\n\\n" + user`` as one
message. This is a real, measured quality risk: verified end-to-end for
``next_question`` in spec Appendix A.3, but the eval gate (spec Phase 4) is what
actually discharges it. Do not flip a production default on this provider before
...
adapter and the host repository (including ``.env``).

Other hazards closed here, each verified in spec Phase 1:

* the prompt goes on **stdin**, never argv, so transcript PII cannot leak via
  ``ps`` (H2), and stdin is a pipe this process closes — never inherited, which
  hung a probe for a full timeout with zero output (H1);
...
construction so a missing binary fails immediately rather than mid-interview.
"""
...
class OpenCodeLLMClient(AnthropicLLMClient):
    """:class:`IntakeLLMClient` backed by the ``opencode`` CLI.

    Overrides only construction and :meth:`_call_json`; see the module docstring
    ...
    def _call_json(self, system: str, user: str) -> Any:
        """Run one ``opencode`` invocation and parse its answer as JSON.

        Same return contract as the parent (parsed JSON via the *inherited*
        ``_extract_json``), so all four interview methods work untouched.
        """
        return _extract_json(self._run(system, user))
    ...
    def _run(self, system: str, user: str) -> str:
        ...
        try:
            ...
            raise IntakeLLMError(f"opencode run timed out after {self._timeout}s") from exc
        except OSError as exc:
            raise IntakeLLMError(f"failed to spawn opencode: {exc}") from exc
        ...
        if not text:
            raise IntakeLLMError(f"opencode returned no assistant text{self._version_suffix()}")
        return text
...
#
# These four functions are duplicated byte-for-byte in the data agent's
# ``opencode_client.py``. The duplication is FORCED by C4: the standalone wheel
