...
**unset** — the constructor supports this with no extra wiring. As a *dev-only*
...
a GovCloud host, and ``http_client`` — e.g.
...
class BedrockLLMClient(AnthropicLLMClient):
    ...
    ``docs/deployment/bedrock-enterprise.md``). ``require_sigv4`` defaults from
    ``BEDROCK_REQUIRE_SIGV4`` when not passed explicitly (D13) — an explicit
