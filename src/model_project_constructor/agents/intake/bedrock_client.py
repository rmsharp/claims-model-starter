...
:class:`AnthropicLLMClient` *pointed at Bedrock*. It overrides only two things:
...
* **Trap 5** (map the SDK's parse / empty-response failures onto the seam's
  ...
  already raise :class:`IntakeLLMError`. ``AnthropicBedrock`` raises the same
  ``anthropic.*`` exception types as the base client (it is the same SDK), so
...
the SDK ships ``AnthropicBedrock`` as a drop-in for ``Anthropic`` — so a thin
...
the boto3 credential chain (env vars / shared profile / IAM role / instance
metadata), mirroring how ``anthropic.Anthropic()`` self-discovers
``ANTHROPIC_API_KEY``. There is therefore **no per-provider key resolver to
wire** — the Phase-A-deferred resolver is a no-op for Bedrock. Pass
...
class BedrockLLMClient(AnthropicLLMClient):
    ...
    an ``anthropic.AnthropicBedrock`` client) and the Bedrock-prefixed default
    model. All four interview methods and the JSON parsing are inherited (see
