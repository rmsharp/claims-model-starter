...
> *Last updated: 2026-07-27 (commit `aca05e2`, after Session 180). This page is a full-rewrite synthesis — not continuously updated. For commits since this date, see `CHANGELOG.md` (maintainer) or `git log`.*
...
**Sessions 2–3 (2026-04-14)** produced `docs/architecture-history/architecture-plan.md` — the canonical architecture plan, nineteen numbered sections whose §14 lays out the six implementation phases — and the v1 payload schemas. Two of its sections became load-bearing invariants:
...
**Exercising the seam — the second provider (Sessions 159–164, 2026-06-17).** E4 built the door; the multi-provider arc walked through it, driven by a target deployment customer on AWS Bedrock with no direct Anthropic contract. The plan (`docs/planning/multi-provider-llm-plan.md`, Session 159) made four strategic decisions, of which one governed everything else: *the eval/parity gate is the load-bearing risk — no provider ships until it clears golden-corpus thresholds.* So the arc built the gate before the provider. Phase A (Session 160) gave each provider its own credential spec — `LLMProviderSpec` / `LLM_PROVIDERS` in `src/model_project_constructor/orchestrator/config.py`, the LLM analogue of `PlatformSpec` / `REPO_PLATFORMS` — preserving every existing caller through a back-compatible `anthropic_api_key` property. Phase B (Session 161) built `tests/eval/`: a golden P&C corpus with pure scorers, a hermetic deterministic tier, and a credential-gated `live` tier. Only then did Phase C (Session 162) add `BedrockLLMClient`, and its shape is the arc's best design decision — because the SDK's Bedrock client is signature-identical to the base client, each Bedrock client is a *thin subclass* of its own package's `AnthropicLLMClient` overriding only construction and the `anthropic.`-prefixed default model, so the hardened `_extract_json` and the seam error classes are inherited and the twin-parser drift surface that §9's O2 guard exists to police is never re-created. Bedrock also authenticates differently: not from a single API-key environment variable but from the AWS credential chain (IAM role, SSO, shared profile, instance metadata) plus a region — which is why its `LLM_PROVIDERS` entry records no key env var at all. Phase D (Session 163) made the provider reachable from the intake web UI (`INTAKE_LLM_PROVIDER` / `INTAKE_LLM_MODEL`) through a closure captured at app construction, leaving the session runner untouched. Phase E (Session 164) closed with the deterministic cutover gate: per-provider pass rates against eight thresholds, under the rule that *an unmeasured threshold cannot certify GO*.

**Harden the gate, never lower it (Sessions 165–177, 2026-06-18/19).** The first live Anthropic baseline (Session 165) failed five of the eight thresholds — and the failures were dominated by harness and corpus artifacts rather than model quality. A first baseline whose own incumbent fails is the calibration signal working; the thresholds stayed *proposed* rather than being lowered to match a harness that could not be trusted. The stretch that followed fixed the harness instead: a robust stakeholder simulator for interview replay, a `max_tokens` truncation guard, N≥5 sampling with transport-error disambiguation, and a governance metric made faithful to the prompt's own "if in doubt, pick the stricter tier" rule — followed by defining the `cycle_time` cadence vocabulary in the governance prompt, which had listed the four cadences with no definitions at all, so the model and the reference labels used the same words differently. Interview convergence and both governance metrics now measure green on the Anthropic baseline; `sql_exec` remains short of threshold, and the Bedrock half has never been measured. The gate's standing verdict is therefore **NO-GO** — `anthropic` remains primary.

**Live Bedrock validation, still open (Sessions 178–180 — 2026-06-19 through 2026-07-25).** Session 178 wrote the operator runbook and three-tier cost estimate for the first live Bedrock run (`docs/planning/bedrock-testing-enablement.md` — roughly a dollar for the governance gate, tens of dollars for the full sweep), and discovered mid-session that AWS's redesigned console had moved the ground under it. **On the unmerged `feat/bedrock-mantle-migration` branch — not on `master`** — Sessions 179–180 then migrated the client to `AnthropicBedrockMantle` (the `bedrock-mantle` endpoint), defaulted it to `anthropic.claude-opus-4-8` — deliberately diverging from the first-party Sonnet default, because the mantle catalog has no Sonnet tier — and added enterprise-networking hooks: `base_url` for a PrivateLink VPC-endpoint or GovCloud host, `http_client` for a forward proxy or corporate TLS-inspection CA bundle, and `require_sigv4`, which turns a stray bearer token into a hard error instead of a silent downgrade from role-based SigV4. `docs/deployment/bedrock-enterprise.md` (branch-only) is the deployment guide for a security-controlled AWS account. That work is unit-green, ruff-clean and mypy-clean — and it is *only* unit-green. On the test account the control plane reported every current-generation Claude model available while the data plane returned 403 "not available for this account" at Input TPM 0, and AWS denied the runtime-quota request on account-eligibility grounds. The blocker is an AWS account, not the code, which is why the branch has not landed and why nothing in this wiki describes the Bedrock path as validated.
...
**Session 93** added an executive-facing Quarto summary (`executive-summaries/`) for business stakeholders — rendered on demand, with the `.qmd` source tracked and the HTML/PDF artifacts deliberately not (the source is the truth; renders are reproducible).
...
By late May the project had shipped a pilot, a feature arc, and dozens of micro-fixes — a natural moment to ask what debt had accumulated. **Session 97 (2026-06-01)** ran a senior-architect technical-debt audit across the whole codebase (read-only, delivered as `audits/2026-06-01-technical-debt-audit.md`): 44 findings — zero critical, 15 moderate, 29 minor — sorted into 37 quick wins and a handful of architectural overhauls. The headline theme: **vocabularies and twin implementations that bypass the type system.** Hand-listed enum members in prompts, duplicated host lists, parallel JSON parsers — each individually harmless, each a silent-drift channel. The program that followed executed essentially all of it:
...
The methodology framework itself evolved across the project. Every session is governed by `SESSION_RUNNER.md` and `SAFEGUARDS.md`; failures teach the framework something the next session inherits. Project-specific institutional memory now lives in `PROJECT_LEARNINGS.md` at the repository root — 61 promoted learnings as of this edition — after **Sessions 124–126 (2026-06-07/08)** migrated the project's learnings table out of the synced framework file (so the upstream `SESSION_RUNNER.md` can be updated without merge conflicts) into a project-owned home. The promotion pipeline has its own discipline: a candidate learning must recur across three sessions before it hardens into a row, and **Session 112's** audit of the candidate roster found (and fixed the process behind) silently-reused candidate numbers.
...
- `src/model_project_constructor/agents/website/` — website agent LangGraph flow (deterministic template-and-graph scaffolding: the one pipeline stage that calls no LLM), dual platform adapters behind the neutral `RepoClient` protocol, governance templates with import-time vocabulary guards, business-case and measurement-plan renderers.
...
- `tests/eval/` — the golden-corpus provider-quality harness: P&C SQL cases, governance reference labels, interview goldens, pure scorers, a hermetic deterministic tier, a credential-gated `live` tier, and the deterministic cutover gate whose standing verdict keeps `anthropic` primary.
- `docs/wiki/claims-model-starter/` — 22 outward-facing wiki pages plus the sidebar, auto-published to the live GitHub Wiki by the tracked `post-commit` hook, with the no-line-citation invariant CI-enforced.
- `docs/methodology/` + `PROJECT_LEARNINGS.md` — the imported framework, project-local conventions, and 61 promoted learnings.
- `docs/architecture-history/` — every archived plan, concept-era and delivered, with dated banners; as of Session 152 every plan delivered up to that point had been archived, emptying `docs/planning/` (the canonical home for active plans), which has since refilled — `multi-provider-llm-plan.md` (Session 159) and `bedrock-testing-enablement.md` (Session 178) on `master`, whose Session-180 rewrite is itself branch-only, plus `httpx-adapter-migration.md` (Session 179, a draft not yet approved for implementation) on the unmerged `feat/bedrock-mantle-migration` branch; `audits/` — the two 2026-06 audits.
- `scripts/` — `scripts/run_pipeline.py` (the end-to-end driver) and `scripts/publish_wiki.sh` (the idempotent wiki publisher).

Open threads as of this edition: **live Bedrock validation is blocked on AWS, not on code** — the client is unit-green, but the test account never received runtime quota for current-generation Claude (the control plane reports the models available while the data plane returns 403 "not available for this account" at Input TPM 0), and AWS denied the quota request on account-eligibility grounds, so the path forward is an enterprise AWS account (`docs/deployment/bedrock-enterprise.md`). That guide and the mantle migration itself sit unmerged on `feat/bedrock-mantle-migration` (§6). Second, **`docs/planning/httpx-adapter-migration.md` proposes replacing both LGPL adapter SDKs (`python-gitlab`, `PyGithub`) with direct `httpx` calls — a draft, neither approved nor executed**: both SDKs remain the shipping dependencies behind the `RepoClient` protocol. Third, the eval gate's `sql_exec` capability still measures short of its threshold on the Anthropic baseline (§6). The older threads — the optional publish-hook citation warning (the CI guard is the primary mechanism) and the standing list of small code-and-doc maintenance items tracked in the session handoffs — remain open. The **`CHANGELOG.md` entry gap for Sessions 114 onward** stays closed: Session 149 refined the changelog cadence convention to gate on shipped-code changes and backfilled the six missing shipped-code entries (Sessions 116–144), so the maintainer ledger is complete under the refined rule (§10).
...
| 2–3 | 2026-04-14 | Foundational | Canonical `docs/architecture-history/architecture-plan.md` (19 sections; §14 = six implementation phases); v1 payload schemas + `HandoffEnvelope` |
...
| 97 | 2026-06-01 | Audit / overhauls | Technical-debt audit — 44 findings, 37 quick wins + architectural overhauls; `audits/` created |
...
| 153 | 2026-06-12 | Methodology (doc-reorg) | Evolution rewrite — CHANGELOG-gap thread retired; Sessions 149–152 accounted for |
| 154 | 2026-06-12 | Methodology | Learnings promotions — rows #59 and #60 |
| 155 | 2026-06-13 | Audit / overhauls | Standing-micros batch — `ModelType` drift pin, vocab-guard consolidation, `measurement_unit` prompt fix, `.claude/` gitignore; GitHub Release `v0.2.0` cut |
| 156 | 2026-06-13 | Methodology | Learnings promotion — row #61 (arm the verifier preamble); GitHub Release `v0.1.0` cut |
| 157 | 2026-06-17 | Methodology (doc-reorg) | **Ghost session** — AI Dependencies wiki page landed via PR #1 and published to the live wiki, with zero session-notes trace |
| 158 | 2026-06-17 | Methodology | Ghost back-fill — Session 157 reconstructed from git/PR/wiki evidence; first Failure Mode #14 realization in project history |
| 159 | 2026-06-17 | Methodology (planning) | `docs/planning/multi-provider-llm-plan.md` — five-phase plan, four strategic decisions, five traps |
| 160 | 2026-06-17 | Platform abstraction | Phase A — `LLMProviderSpec` / `LLM_PROVIDERS` credential seam; the pilot `--model` default named as `PILOT_DEFAULT_MODEL` |
| 161 | 2026-06-17 | CI / quality gates | Phase B — `tests/eval/` corpus, scorers, and the hermetic/live two-tier battery |
| 162 | 2026-06-17 | Platform abstraction | Phase C — `BedrockLLMClient` in both packages as a thin subclass; `LLMProvider` gains `"bedrock"`; live Bedrock run deferred (no AWS credentials) |
| 163 | 2026-06-17 | Platform abstraction | Phase D — intake web UI provider/model selection (`INTAKE_LLM_PROVIDER` / `INTAKE_LLM_MODEL`) |
| 164 | 2026-06-17 | CI / quality gates | Phase E — provider shadow run + deterministic cutover gate; with no credentials every threshold reads PENDING |
| 165 | 2026-06-18 | CI / quality gates | Live Anthropic baseline + shadow-run driver — 5 of 8 thresholds fail on harness/corpus artifacts; decision **NO-GO**, `anthropic` stays primary |
| 166–171 | 2026-06-18 | CI / quality gates | Eval-harness hardening — stakeholder simulator; `max_tokens` → 16384 + truncation guard; convergence-scorer calibration, N≥5 sampling, transport-error robustness; convergence gate live-GREEN |
| 172–177 | 2026-06-19 | CI / quality gates | Quarto convergence explainer; governance metric made faithful; SME-ratified references; `cycle_time` cadence vocabulary defined in `SYSTEM_GOVERNANCE`; `tactical`/`operational` re-anchored on output purpose + role≠frequency corpus case → live `cycle_time` 60/60 |
| 178 | 2026-06-19 | Methodology (planning) | `docs/planning/bedrock-testing-enablement.md` — operator runbook + three-tier cost estimate; redesigned-console flow verified (last session on `master`) |
| 179–180 | 2026-07-17/25 | Platform abstraction | **On the unmerged `feat/bedrock-mantle-migration` branch** — httpx adapter-migration plan (Session 179, a draft); `AnthropicBedrockMantle` swap, `anthropic.claude-opus-4-8` default, enterprise `base_url` / `http_client` / `require_sigv4` hooks and `docs/deployment/bedrock-enterprise.md` (Session 180); live path blocked by an AWS account-eligibility / runtime-quota gate |
| 181 | 2026-07-27 | Methodology (doc-reorg) | This refresh — wiki content resynced to Session 180; the multi-provider/Bedrock arc folded into §6, §11 and this index |
...
- `docs/architecture-history/architecture-plan.md` — Session 2 canonical plan, nineteen numbered sections. §7 decoupling invariant; §8 governance-artifact framework; §14 implementation phases and acceptance criteria (all verified in Session 17).
...
`docs/planning/` — the canonical location for active plans — was emptied by Sessions 147, 151 and 152, each moved plan carrying a dated banner per the planning-doc archive convention in `docs/methodology/PROJECT_CONVENTIONS.md`. It has since refilled:

- `docs/planning/multi-provider-llm-plan.md` — the five-phase multi-provider LLM plan (§6); Phases A–E delivered in Sessions 160–164.
- `docs/planning/bedrock-testing-enablement.md` — the operator runbook and cost estimate for a live Bedrock run (Session 178, on `master`; its Session-180 rewrite to the verified mantle reality and the AWS quota blocker is on the unmerged `feat/bedrock-mantle-migration` branch).
- `docs/planning/httpx-adapter-migration.md` — a draft, not yet approved for implementation, proposing that both repo-host adapters drop their LGPL SDKs for direct `httpx` calls (Session 179); on the unmerged `feat/bedrock-mantle-migration` branch.

Other current documentation homes:

- `docs/explainers/interview-convergence-explainer.qmd` (with its rendered PDF) — how the live interview-convergence test works (Session 172).
- `docs/deployment/bedrock-enterprise.md` — the enterprise Bedrock deployment guide (least-privilege IAM, SigV4/role auth, PrivateLink, data residency, pre-deployment checklist); on the unmerged `feat/bedrock-mantle-migration` branch.

Audits live at `audits/`: the 2026-06-01 technical-debt audit (spawned the §9 program) and the 2026-06-10 wiki-vs-code accuracy audit (spawned the §10 citation campaign).
...
- `PROJECT_LEARNINGS.md` (repo root) — the project's 61 promoted institutional learnings.
