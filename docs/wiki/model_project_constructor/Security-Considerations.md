...
| `INTAKE_LLM_PROVIDER` | no | `anthropic` | LLM backend for the intake web UI: `anthropic`, `bedrock`, or `opencode`. An unknown value raises `ValueError` at app construction (`_resolve_provider` in `src/model_project_constructor/ui/intake/app.py`). Only read by the UI; the CLIs use `--provider`. |
...
LLM client construction is routed through factory functions (`make_llm_client` in `src/model_project_constructor/agents/intake/factory.py` for Intake; `make_llm_client` in `packages/data-agent/src/model_project_constructor_data_agent/factory.py` for Data Agent) so the provider is named explicitly by callers, and a second LLM backend becomes one new client module plus one branch in the factory — no changes at the call sites. The known-provider list is single-sourced via `Literal` + `get_args`, so unknown-provider errors cannot drift from reality. Three providers are implemented: `"anthropic"` (the first-party API, keyed by `ANTHROPIC_API_KEY`), `"bedrock"` (AWS-hosted Claude, authenticated by the AWS credential chain rather than an API-key env var — see `src/model_project_constructor/agents/intake/bedrock_client.py` and §1.2 above), and `"opencode"` (the `opencode` CLI driven as a subprocess — see `opencode_client.py` in both packages). Both factories single-source the list from `LLMProvider = Literal["anthropic", "bedrock", "opencode"]`. The orchestrator's `LLM_PROVIDERS` registry records `bedrock` and `opencode` with `api_key_env_var=None`, so `require_llm_api_key` deliberately raises for either rather than inventing an env-var key.

**Two security properties of the `opencode` provider that have no analogue in the SDK providers**, both stated here because a reviewer will not infer them from the code's shape. (1) **The network destination is not determinable from this repository.** The client spawns a local binary; which vendor and endpoint that binary reaches is decided entirely by the operator's own OpenCode configuration, and this project neither reads nor records the credential involved. (2) **The subprocess is an agent, not a completion endpoint**, so it is capable of filesystem access. The client's containment is an ephemeral working directory plus a generated agent definition that denies the `edit`, `write`, `bash`, `read`, and `webfetch` tools, always applied with no constructor path that bypasses it, and the auto-approve flag is never passed. That denial is **load-bearing, not defence-in-depth**: live verification found that without it a run read a file and disclosed its contents, exiting cleanly. Prompt text is written to the child's stdin and never appears in `argv`, so transcripts are not exposed via the process table. See [AI Dependencies §6.9](AI-Dependencies#69-the-opencode-provider-a-subprocess-agent-in-the-call-path) for the full treatment, including the machine-global session store this provider writes prompt and response text into.
