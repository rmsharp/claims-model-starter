...
| **IntakeReport** | Structured output of the Intake Agent: business problem, proposed solution, model solution, estimated value, value measurement plan, governance metadata. |
...
| **Checkpoint** | A persisted JSON envelope representing a completed inter-agent handoff. Used for inspection and resumption of interrupted runs. |
| **Run ID** | A free-form string identifier for a single pipeline execution, auto-generated by the CLI (default format: `run_<8 hex chars>`). All checkpoints for one run share the same run ID. |
