...
> *Last updated: 2026-07-27 (commit `a1d8af7`, Session 181; content resynced through Session 180). This page is a full-rewrite synthesis — not continuously updated. For commits since this date, see `CHANGELOG.md` (maintainer) or `git log`.*
...
**Live Bedrock validation, still open (Sessions 178–180 — 2026-06-19 through 2026-07-25).** Session 178 wrote the operator runbook and three-tier cost estimate for the first live Bedrock run (`docs/planning/bedrock-testing-enablement.md` — roughly a dollar for the governance gate, tens of dollars for the full sweep), and discovered mid-session that AWS's redesigned console had moved the ground under it. Sessions 179–180 then migrated the client to `AnthropicBedrockMantle` (the `bedrock-mantle` endpoint), defaulted it to `anthropic.claude-opus-4-8` — deliberately diverging from the first-party Sonnet default, because the mantle catalog has no Sonnet tier — and added enterprise-networking hooks: `base_url` for a PrivateLink VPC-endpoint or GovCloud host, `http_client` for a forward proxy or corporate TLS-inspection CA bundle, and `require_sigv4`, which turns a stray bearer token into a hard error instead of a silent downgrade from role-based SigV4. `docs/deployment/bedrock-enterprise.md` is the deployment guide for a security-controlled AWS account. That work is unit-green, ruff-clean and mypy-clean — and it is *only* unit-green. On the test account the control plane reported every current-generation Claude model available while the data plane returned 403 "not available for this account" at Input TPM 0, and AWS denied the runtime-quota request on account-eligibility grounds. The blocker was an AWS account, not the code, and the Bedrock path has never been exercised against live AWS.
...
- `docs/wiki/model_project_constructor/` — 24 outward-facing wiki pages plus the sidebar, auto-published to the live GitHub Wiki by the tracked `post-commit` hook, with the no-line-citation invariant CI-enforced.
- `docs/methodology/` + `PROJECT_LEARNINGS.md` — the imported framework, project-local conventions, and 61 promoted learnings.
- `docs/architecture-history/` — every archived plan, concept-era and delivered, with dated banners; as of Session 152 every plan delivered up to that point had been archived, emptying `docs/planning/` (the canonical home for active plans), which has since refilled — `multi-provider-llm-plan.md` (Session 159), `bedrock-testing-enablement.md` (Session 178, rewritten Session 180), and `httpx-adapter-migration.md` (Session 179, a draft not yet approved for implementation); `audits/` — the two 2026-06 audits.
- `scripts/` — `scripts/run_pipeline.py` (the end-to-end driver) and `scripts/publish_wiki.sh` (the idempotent wiki publisher).

Open threads as of this edition: **live Bedrock validation is blocked on AWS, not on code** — the client is unit-green, but the test account never received runtime quota for current-generation Claude (the control plane reports the models available while the data plane returns 403 "not available for this account" at Input TPM 0), and AWS denied the quota request on account-eligibility grounds, so the path forward is an enterprise AWS account (`docs/deployment/bedrock-enterprise.md`). Second, **`docs/planning/httpx-adapter-migration.md` proposes replacing both LGPL adapter SDKs (`python-gitlab`, `PyGithub`) with direct `httpx` calls — a draft, neither approved nor executed**: both SDKs remain the shipping dependencies behind the `RepoClient` protocol. Third, the eval gate's `sql_exec` capability still measures short of its threshold on the Anthropic baseline (§6). The older threads — the optional publish-hook citation warning (the CI guard is the primary mechanism) and the standing list of small code-and-doc maintenance items tracked in the session handoffs — remain open. The **`CHANGELOG.md` entry gap for Sessions 114 onward** stays closed: Session 149 refined the changelog cadence convention to gate on shipped-code changes and backfilled the six missing shipped-code entries (Sessions 116–144), so the maintainer ledger is complete under the refined rule (§10).
...
| 178 | 2026-06-19 | Methodology (planning) | `docs/planning/bedrock-testing-enablement.md` — operator runbook + three-tier cost estimate; redesigned-console flow verified |
| 179–180 | 2026-07-17/25 | Platform abstraction | httpx adapter-migration plan (Session 179, a draft); `AnthropicBedrockMantle` swap, `anthropic.claude-opus-4-8` default, enterprise `base_url` / `http_client` / `require_sigv4` hooks and `docs/deployment/bedrock-enterprise.md` (Session 180); live path blocked by an AWS account-eligibility / runtime-quota gate |
...
- `docs/planning/bedrock-testing-enablement.md` — the operator runbook and cost estimate for a live Bedrock run (Session 178; rewritten in Session 180 to the verified mantle reality and the AWS quota blocker).
- `docs/planning/httpx-adapter-migration.md` — a draft, not yet approved for implementation, proposing that both repo-host adapters drop their LGPL SDKs for direct `httpx` calls (Session 179).
...
- `docs/deployment/bedrock-enterprise.md` — the enterprise Bedrock deployment guide (least-privilege IAM, SigV4/role auth, PrivateLink, data residency, pre-deployment checklist).
