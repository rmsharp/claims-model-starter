# Session Notes — archive shard: Sessions 220 → 217

**Frozen. Write-once. Do not edit this file.** It holds the record byte spans that the second trim
(Session 224, 2026-08-19) removed from `SESSION_NOTES.md`: Sessions 220 → 217 — five record
headings, 774 lines — in the same newest-on-top order, byte-for-byte unedited.

**This shard is not the whole history.** Sessions 216 → 1 are in a different, earlier shard,
`SESSION_NOTES-through-S216.md`. Neither file is a prefix of the other; neither is authoritative
alone. Session N lives in: that earlier shard when N ≤ 216; **this** file when 217 ≤ N ≤ 220; the
live ledger when N ≥ 221.

**`grep` this file; do not `Read` it.** At 804 lines it sits under the 2,000-line agent read cap
...
contain it). That size is final: shards are write-once, so a third trim creates a third file rather
than growing this one.

**Provenance is proved here, not asserted.** Run `SESSION_NOTES-S220-through-S217.md.verify.sh`,
beside this file. It re-derives L0/L1/L2/L3 from git, adds an **L4 this cut introduces** that pins
the boundary itself, exits non-zero on failure, and under `--self-test` mutates the artifacts
fifteen ways and fails if any mutation goes uncaught. Do not believe this paragraph — run it.

**Cutting is by byte position, never by authorship.** Session 220's handoff evaluation is filed
inside Session 221's record, because this ledger files an evaluation under its author — so that
evaluation stayed behind in the live file while Session 220's own record came here. The same seam
brought Session 216's evaluation INTO this file, inside Session 217's record, out of the live file
where the first trim had left it. Expect this at every boundary, in both directions.
