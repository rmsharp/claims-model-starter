...
class EstimatedValue(BaseModel):
    ...
    # Pre-construction business-case fields (business-value-capture-plan.md §4.1).
    # All optional at the schema level; required for COMPLETE status at finalize-time.
...
class ValueMeasurementPlan(BaseModel):
    """Production-demonstration contract (business-value-capture-plan.md §4.2).

    Captured at intake; consumed by the data agent (which executes the baseline
    SQL) and surfaced by the website agent (which renders the production
    measurement plan section of ``06_implementation_plan.qmd``)."""
    ...
    decision_rights: str | None = None       # Advisory; no state-machine enforcement
...
class IntakeReport(BaseModel):
    ...
    # Four required business sections (from initial_purpose.txt) plus the
    # post-construction measurement plan (business-value-capture-plan.md §3.3)
...
class BaselineSnapshot(BaseModel):
    """Captured by the data agent when intake provides a ValueMeasurementPlan
    (business-value-capture-plan.md §4.3). Sourced from a baseline query
    generated by the data agent against the production database."""
    metric_name: str                   # Mirrors ValueMeasurementPlan.baseline_metric_name
    value: float | None                # None when query not executed or failed
    ...
    query_sql: str                     # The SQL the data agent generated
...
class DataReport(BaseModel):
    ...
    created_at: datetime
    baseline_snapshot: BaselineSnapshot | None = None  # Production-demonstration baseline
```
...
> **Note (Phase 1, Session 86):** `DataReport.baseline_snapshot` is populated by the data agent when `IntakeReport.value_measurement_plan` is present (business-value-capture-plan.md Phase 1). Subsequent phases of the plan wire the data agent to generate and execute the baseline query (Phase 3) and the website agent to surface the snapshot in the generated implementation-plan document (Phase 5).
