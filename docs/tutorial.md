...
Clone the repository (substitute your clone URL):
...
Set the `MPC_NAMESPACE` environment variable to control where the project is created. **`MPC_NAMESPACE` must be a path, not a URL** — the GitLab and GitHub adapters look up the group/org by path, so pasting the full URL surfaces as an unhelpful `404`. The orchestrator rejects URL-prefixed values at config-load time. Pick the form that matches your host:
...
For self-hosted GitLab or GitHub Enterprise, set the host URL for your platform:
...
**Provider selection (`--provider`).** `--provider` (default `anthropic`) selects the LLM backend and, like `--model`, applies to **both** stages. Construction routes through each agent's `make_llm_client` factory. Three providers ship today — `anthropic`, `bedrock` (Claude via AWS Bedrock), and `opencode` (the `opencode` CLI driven as a subprocess, so the underlying vendor is whatever that tool is configured for) — proof that the factory's forward-looking seam (audit §E4) works: each is one new client module plus one factory branch, with no change at the call sites. Neither non-default provider is validated: `bedrock` has never been exercised against a live endpoint, and `opencode` runs but its output quality is unmeasured, so `anthropic` remains the default. `opencode` additionally needs its binary on `PATH` and an explicit `--model` (it pins no default). An unknown provider fails fast (`ValueError`) before any API call. The flag is also exposed on the standalone data-agent CLI (`model-data-agent run`/`discover --provider`).
