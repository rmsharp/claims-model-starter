...
- `SESSION_NOTES.md` — Session continuity: active task, handoff notes, session history. **Trimmed twice (Sessions 222, 224):** holds the newest 4 sessions. Retired records live in **two** frozen shards under `docs/architecture-history/` — Sessions 216→1 in `SESSION_NOTES-through-S216.md`, Sessions 220→217 in `SESSION_NOTES-S220-through-S217.md`. **`grep` those shards, never `Read` them**, and grep *both* — neither is a prefix of the other. See "SESSION_NOTES.md is trimmed" below.
...
### `SESSION_NOTES.md` is trimmed (Sessions 222, 224)
...
- **Shards + proofs — there are TWO, and a lookup must consult both.** `SESSION_NOTES-through-S216.md` (Sessions 216→1, 24,590 lines) and `SESSION_NOTES-S220-through-S217.md` (Sessions 220→217, 804 lines), each beside its own `.verify.sh`. **Session N is in:** the S216 shard when N ≤ 216; the S220 shard when 217 ≤ N ≤ 220; the live file when N ≥ 221. **`grep` them; never `Read` them** — and *nothing watches either*: the dashboard's `READ_CAP_WATCHED` is an exact-path set containing neither. (The S220 shard is under the 2,000-line cap today; that is luck, not protection.) Shards are **write-once**: a new trim writes a new file with a new cut key and never appends to an existing one — which is why the second shard's name is a *range*, `-S220-through-S217`, not the first's open-ended `-through-S216`.
