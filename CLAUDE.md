...
- `SESSION_NOTES.md` — Session continuity: active task, handoff notes, session history. **Trimmed three times (Sessions 222, 224, 228):** holds the newest 4 sessions. Retired records live in **three** write-once shards under `docs/architecture-history/`. **`grep` them, never `Read` them**, and grep *all three* — none is a prefix of another. The routing table that says which shard holds Session N is in the bullet below; it is deliberately stated once here rather than restated in this line. See "SESSION_NOTES.md is trimmed" below.
...
### `SESSION_NOTES.md` is trimmed (Sessions 222, 224, 228)
...
- **Shards + proofs — there are THREE, and a lookup must consult all of them.** `SESSION_NOTES-through-S216.md` (Sessions 216→1, 24,590 lines), `SESSION_NOTES-S220-through-S217.md` (220→217, 804 lines) and `SESSION_NOTES-S224-through-S221.md` (224→221, 933 lines), each beside its own `.verify.sh`. **Session N is in:** **N ≤ 216** → `SESSION_NOTES-through-S216.md`; **217 ≤ N ≤ 220** → `SESSION_NOTES-S220-through-S217.md`; **221 ≤ N ≤ 224** → `SESSION_NOTES-S224-through-S221.md`; **N ≥ 225** → `SESSION_NOTES.md`. **That table is written in the canonical clause form on purpose: the third trim's L5 parses it out of THIS file and checks it — against the cut key, and against the record ids the named files actually contain. It is the one copy here that cannot silently go stale.** **`grep` them; never `Read` them** — and *nothing watches any of them*: the dashboard's `READ_CAP_WATCHED` is an exact-path set containing none. (The two newer shards are under the 2,000-line cap today; that is luck, not protection.) Shards are **write-once**: a new trim writes a new file with a new cut key and never appends to an existing one — which is why every shard after the first is named as a *range*, not the first's open-ended `-through-S216`. **L5 reads three copies — the live pointer block, the shard banner and this bullet. TWO more sit outside every proof's reach:** `README.md`'s repo map and the shard-naming rule in `docs/methodology/PROJECT_CONVENTIONS.md`. Update both in the trim commit or they lie from the next cut onward. And note what L5 cannot help with: a copy inside a **write-once** shard banner is frozen the moment the next trim lands — the S220 shard's banner already says `N ≥ 221` → the live file, which Session 228 falsified and no one may repair. A shard banner is a snapshot of its own cut; the live pointer block is the authority.
